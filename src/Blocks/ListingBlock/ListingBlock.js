...
function ListingBlock({ getListing, getUpdates }) {
  const [overlay, setOverlay] = useState({});

  const listing = getListing && getListing();
  const updates = getUpdates && getUpdates();
    ...
    <div>
      ...
      <div className="bb-grid bb-grid-cols-4 bb-gap-3">
        <div className="bb-col-span-3">
          ...
          {getUpdates && <Updates pathname={`/listings/${listing.slug}`} getUpdates={getUpdates} />}
        </div>
